...
function ciniki_ags_itemUpdate(&$ciniki) {
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'sapos_category'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Accounting Category'),
    ...
    if( isset($args['subcategories']) ) {
        ...
        $rc = ciniki_core_tagsUpdate($ciniki, 'ciniki.ags', 'itemtag', $args['tnid'],
            'ciniki_ags_item_tags', 'ciniki_ags_history',
            'item_id', $args['item_id'], 30, $args['subcategories']);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.ags');
            return $rc;
        }
    ...
    if( isset($args['tags']) ) {
        ...
        $rc = ciniki_core_tagsUpdate($ciniki, 'ciniki.ags', 'itemtag', $args['tnid'],
            'ciniki_ags_item_tags', 'ciniki_ags_history',
            'item_id', $args['item_id'], 60, $args['tags']);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.ags');
            return $rc;
        }
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'wng', 'indexObject', array('object'=>'ciniki.ags.item', 'object_id'=>$args['item_id']));
