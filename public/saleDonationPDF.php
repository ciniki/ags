...
// -------
//
function ciniki_ags_saleDonationPDF(&$ciniki) {
    //  
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'sale_id'=>array('required'=>'yes', 'blank'=>'no', 'name'=>'Sale'), 
    ...
    $rc = ciniki_ags_checkAccess($ciniki, $args['tnid'], 'ciniki.ags.saleDonationPDF'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    //
    // Open the donation receipt function
    //
    $invoice_template = 'donationreceipt';
    $rc = ciniki_core_loadMethod($ciniki, 'ciniki', 'ags', 'templates', 'donationreceipt');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $fn = $rc['function_call'];
...
/*    if( isset($args['email']) && $args['email'] == 'yes' ) {
        $rc = $fn($ciniki, $args['tnid'], $args['sale_id'],
        ...
        if( !isset($invoice['customer']) ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.ags.262', 'msg'=>'No customer attached to the invoice, we are unable to send the email.'));
        }
        if( !isset($invoice['customer']['emails'][0]['email']['address']) ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.ags.263', 'msg'=>"The customer doesn't have an email address, we are unable to send the email."));
        }
        ...
        if( isset($args['subject']) && isset($args['textmsg']) ) {
            ...
            $subject = 'Donation #' . $sale['receipt_number'];
        ...
        if( isset($sapos_settings['invoice-email-all-addresses']) && $sapos_settings['invoice-email-all-addresses'] == 'yes' ) {
            foreach($invoice['customer']['emails'] as $e) {
                ...
                if( $rc['stat'] != 'ok' ) {
                    ...
                    return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.ags.265', 'msg'=>'Unable to create mail message.', 'err'=>$rc['err']));
                }
            ...
            if( $rc['stat'] != 'ok' ) {
                ciniki_core_dbTransactionRollback($ciniki, 'ciniki.mail');
                return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.ags.266', 'msg'=>'Unable to create mail message.', 'err'=>$rc['err']));
            }
            $ciniki['emailqueue'][] = array('mail_id'=>$rc['id'], 'tnid'=>$args['tnid']);
        }
    ...
    return $fn($ciniki, $args['tnid'], $args['sale_id'], $tenant_details, $sapos_settings);
}
?>
