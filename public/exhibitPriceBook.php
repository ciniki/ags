...
// Description
// ===========
// This method will generate a PDF of the prices organized by Type for an exhibit.
// By default only items that are untagged will be included.
//
// Arguments
// ---------
// api_key:
...
// -------
//
function ciniki_ags_exhibitPriceBook($ciniki) {
    //  
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'tagged'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Included Tagged Items'),
    ...
    //
    // Get the list of exhibit items, sorted by type
    //
    $strsql = "SELECT ciniki_ags_exhibit_items.id, "
        ...
        . "items.flags, "
        . "items.unit_amount, "
        . "IFNULL(tags.tag_name, 'Misc') AS type "
        ...
        . "";
    if( !isset($args['tagged']) || $args['tagged'] != 'yes' ) {
        $strsql .= "AND (items.flags&0x10) = 0 ";
    }
    $strsql .= "ORDER BY type, code, name "
        . "";
    ...
    ciniki_core_loadMethod($ciniki, 'ciniki', 'ags', 'templates', 'pricebook');
    $rc = ciniki_ags_templates_pricebook($ciniki, $args['tnid'], array(
        'title'=>$exhibit_name . ' - Price Book',
    ...
    // Output the pdf
    //
    $filename = $exhibit_name . ' - Price Book - ' . $today->format('M d, Y');
