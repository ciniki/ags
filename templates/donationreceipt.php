...
// Description
// ===========
// This method will produce a PDF of the donation receipt.
//
// Arguments
// ---------
// 
...
// <rsp stat='ok' id='34' />
//
function ciniki_ags_templates_donationreceipt(&$ciniki, $tnid, $sale_id, $tenant_details, $sapos_settings, $output='download') {
    
    //
    // Load the details about the sale
    //
    $strsql = "SELECT sales.id, "
        . "sales.sell_date, "
        . "YEAR(sales.sell_date) AS donation_year, "
        . "sales.tenant_amount, "
        . "sales.receipt_number, "
        . "items.donor_customer_id, "
    ...
    if( is_array($sale_id) ) {
        ...
        $strsql .= "WHERE sales.id IN (" . ciniki_core_dbQuoteIDs($ciniki, $sale_id) . ") ";

    } else {
        $strsql .= "WHERE sales.id = '" . ciniki_core_dbQuote($ciniki, $sale_id) . "' ";
    }
    $strsql .= "AND sales.tnid = '" . ciniki_core_dbQuote($ciniki, $tnid) . "' "
        . "";
    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'dbHashQueryArrayTree');
    $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.ags', array(
        array('container'=>'sales', 'fname'=>'id', 
            'fields'=>array('id', 'sell_date', 'donation_year', 'tenant_amount', 'receipt_number', 'donor_customer_id', 'customer_id'),
            ),
        ));
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.ags.247', 'msg'=>'Unable to load item_sales', 'err'=>$rc['err']));
    }
    ...
    foreach($sales as $sid => $sale) {
        if( $sale['donor_customer_id'] > 0 ) {
            $donor_customer_id = $sale['donor_customer_id'];
        } elseif( $sale['customer_id'] > 0 ) {
            $donor_customer_id = $sale['customer_id'];
        } else {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.ags.246', 'msg'=>'Unable to find donor'));
        }
        ...
        $sales[$sid]['addr'] = array();
        if( isset($rc['customer']) ) {
            $sales[$sid]['addr'][] = $rc['customer']['display_name'];
            $address = isset($rc['customer']['addresses'][0]) ? $rc['customer']['addresses'][0] : array();
            if( isset($address['address1']) && $address['address1'] != '' ) {
                $sales[$sid]['addr'][] = $address['address1'];
            }
            if( isset($address['address2']) && $address['address2'] != '' ) {
                $sales[$sid]['addr'][] = $address['address2'];
            }
            $city = '';
            ...
            if( isset($address['country']) && $address['country'] != '' ) {
                $sales[$sid]['addr'][] = $address['country'];
            }
        }
    }
...
//                        0, $this->header_height-5, 'JPEG', '', 'L', 2, '150', '', false, false, 1);
    ...
    $donation_amount = $sale['tenant_amount'];
    ...
    foreach($sales as $sid => $sale) {
        if( $donation_amount >= $donation_minimum ) {
            ...
            $pdf->SetFont('', '');
            $pdf->Cell($w[1], $lh, $sale['receipt_number'], 0, 0, 'L', 1);
            $pdf->Cell($w[2], $lh, (isset($sale['addr'][0])?$sale['addr'][0]:''), 0, 0, 'L', 1);
            $pdf->Ln();
            ...
            $pdf->Cell($w[2], $lh, (isset($sale['addr'][1])?$sale['addr'][1]:''), 0, 0, 'L', 1);
            $pdf->Ln();

            $pdf->SetFont('', 'B');
            $pdf->Cell($w[0], $lh, 'Date Received:', 0, 0, 'R', 1);
            $pdf->SetFont('', '');
            $pdf->Cell($w[1], $lh, $sale['sell_date'], 0, 0, 'L', 1);
            $pdf->Cell($w[2], $lh, (isset($sale['addr'][2])?$sale['addr'][2]:''), 0, 0, 'L', 1);
            $pdf->Ln();
            ...
            $pdf->Cell($w[2], $lh, (isset($sale['addr'][3])?$sale['addr'][3]:''), 0, 0, 'L', 1);
            $pdf->Ln();
            ...
            $pdf->Cell($w[2], $lh, (isset($sale['addr'][4])?$sale['addr'][4]:''), 0, 0, 'L', 1);
            $pdf->Ln();
            ...
            $pdf->Ln(10);
            $pdf->Cell(180, $lh, 'Official ' . $sale['donation_year'] . ' Donation Receipt for Income Tax Purposes, Canada Revenue Agency: canada.ca/charities-giving', 0, 0, 'L', 1);
            $pdf->Ln(10);
    ...
    if( $output == 'email' ) {
        return array('stat'=>'ok', 'filename'=>'receipt_' . $sale['receipt_number'] . '.pdf', 'pdf'=>$pdf, 'sale'=>$sale);
    } else {
